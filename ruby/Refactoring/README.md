...
* 条件分岐を取り除くことを目的とする
* クラスのふるまいに影響を与えるタイプコードが使われている時に用いる
* タイプコードをクラスに書き換える。タイプコードの値１つに１つのクラスを作る

#### タイプコードからモジュールのextendへ

* 「タイプコードからポリモーフィズムへ」と同様条件分岐を取り除くことを目的とする
* モジュールを簡単にはアンミックスできなくなるという弱点がる

#### タイプコードからState/Strategyへ

* 「タイプコードからポリモーフィズムへ」「タイプコードからextendへ」と同様条件分岐を取り除くことを目的とする
* extendでは手に負えない時
